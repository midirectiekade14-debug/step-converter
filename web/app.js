...
async function handleFile(file) {
  ...
  try {
    ...
    setupPreview();
    updateMeshInfo();
    if (bridgeConnected) btnSendToSU.disabled = false;
...
// --- SketchUp Live Bridge ---
const BRIDGE_URL = 'http://127.0.0.1:7891';
const bridgeDot = document.getElementById('bridgeDot');
const bridgeStatusText = document.getElementById('bridgeStatusText');
const btnSendToSU = document.getElementById('btnSendToSU');
const btnImportFromSU = document.getElementById('btnImportFromSU');
...
async function checkBridge() {
  try {
    ...
    if (resp.ok) {
      ...
      bridgeStatusText.textContent = `Verbonden (SU ${data.sketchup || '?'})`;
      btnImportFromSU.disabled = false;
      btnSendToSU.disabled = !parsedData;
      return;
    }
  } catch (e) { /* not available */ }
  bridgeConnected = false;
  bridgeDot.classList.remove('connected');
  bridgeStatusText.textContent = 'Niet verbonden';
  btnSendToSU.disabled = true;
  btnImportFromSU.disabled = true;
}
...
// Send current model to SketchUp
btnSendToSU.addEventListener('click', async () => {
  if (!parsedData || !bridgeConnected) return;
  btnSendToSU.disabled = true;
  btnSendToSU.textContent = 'Versturen...';
  try {
    const objBlob = exportOBJ();
    const objText = await objBlob.text();
    ...
    if (resp.ok) {
      ...
      btnSendToSU.textContent = `Verstuurd! (${result.groups || '?'} groepen)`;
    } else {
      const err = await resp.text();
      showError(`SketchUp bridge fout: ${err}`);
      btnSendToSU.textContent = 'Stuur naar SketchUp';
    }
  } catch (e) {
    showError(`Kan niet verbinden met SketchUp: ${e.message}`);
    btnSendToSU.textContent = 'Stuur naar SketchUp';
  }
  setTimeout(() => {
    btnSendToSU.textContent = 'Stuur naar SketchUp';
    btnSendToSU.disabled = !bridgeConnected || !parsedData;
  }, 3000);
});

// Import model from SketchUp
btnImportFromSU.addEventListener('click', async () => {
  if (!bridgeConnected) return;
  btnImportFromSU.disabled = true;
  btnImportFromSU.textContent = 'Importeren...';
  try {
    const resp = await fetch(`${BRIDGE_URL}/export`, { signal: AbortSignal.timeout(30000) });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const objText = await resp.text();
    if (!objText.trim() || !objText.includes('v ')) {
      showError('SketchUp model is leeg — geen geometrie gevonden.');
      btnImportFromSU.textContent = 'Importeer uit SketchUp';
      btnImportFromSU.disabled = false;
      return;
    }
    // Parse the OBJ from SketchUp as a virtual file
    const blob = new Blob([objText], { type: 'text/plain' });
    const file = new File([blob], 'sketchup_export.obj', { type: 'text/plain' });
    loadedFile = file;
    fileName.textContent = `sketchup_export.obj (vanuit SketchUp)`;
    convertBtn.disabled = false;

    const buffer = await blob.arrayBuffer();
    await handleModelFile(buffer, 'sketchup_export.obj');
    showProgress('3D preview laden...', 80);
    setupPreview();
    updateMeshInfo();
    showProgress('SketchUp model geladen!', 100);
    setTimeout(() => progress.classList.remove('active'), 500);
    btnImportFromSU.textContent = 'Geimporteerd!';
  } catch (e) {
    showError(`Import uit SketchUp mislukt: ${e.message}`);
    btnImportFromSU.textContent = 'Importeer uit SketchUp';
  }
  setTimeout(() => {
    btnImportFromSU.textContent = 'Importeer uit SketchUp';
    btnImportFromSU.disabled = !bridgeConnected;
  }, 3000);
});
